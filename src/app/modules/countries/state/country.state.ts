...
export class CountryStateModel {
  ...
  public CountriesAll: CountryModel[];
  public countryDetails: CountryModel | null;
...
export class countryState {
  ...
  /* ________________________ SELECTORS __________________________*/
  @Selector()
  ...
  /* ______________________  REDUCERS ______________________*/
  ...
  public getCountryByName(ctx: StateContext<CountryStateModel>, payload: GetCountryByName) {
    return this._countryService.countryDetails(payload.name).pipe(
      tap(countryDetails => {
        ctx.patchState({ countryDetails: countryDetails[0] });
      })
    )
  }
